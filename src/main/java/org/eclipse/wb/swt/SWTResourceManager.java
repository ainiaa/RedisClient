...
public class SWTResourceManager {
     ...
     */
    private static final Map<String, Image> m_imageMap = new HashMap<>();
    ...
    public static void disposeImages() {
        ...
        {
        m_imageMap.values().stream().forEach((image) -> {
            image.dispose();
            });
            m_imageMap.clear();
        }
        ...
        for (Map<Image, Map<Image, Image>> cornerDecoratedImageMap : m_decoratedImageMap) {
            if (cornerDecoratedImageMap != null) {
                cornerDecoratedImageMap.values().stream().map((decoratedMap) -> {
                    decoratedMap.values().stream().forEach((image) -> {
                        image.dispose();
                    });
                    return decoratedMap;
                }).forEach((decoratedMap) -> {
                    decoratedMap.clear();
                });
     ...
     */
    private static final Map<String, Font> m_fontMap = new HashMap<>();
    /**
     * Maps fonts to their bold versions.
     */
    private static final Map<Font, Font> m_fontToBoldFontMap = new HashMap<>();
    ...
    public static void disposeFonts() {
        // clear fonts
        m_fontMap.values().stream().forEach((font) -> {
            font.dispose();
        });
        m_fontMap.clear();
        // clear bold fonts
        m_fontToBoldFontMap.values().stream().forEach((font) -> {
            font.dispose();
        });
